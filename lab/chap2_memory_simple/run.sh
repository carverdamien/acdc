...
baseline)
MEMORY=$((MEMORY*2))
RUNA() { ${RUN} exec -T filebencha job python benchmark.py -- filebencha -f workloads/A/automatic.f;}
RUNB() { ${RUN} exec -T filebenchb job python benchmark.py -- filebenchb -f workloads/B/automatic.f;}
;;
automatic)
RUNA() { ${RUN} exec -T filebencha job python benchmark.py -- filebencha -f workloads/A/automatic.f;}
RUNB() { ${RUN} exec -T filebenchb job python benchmark.py -- filebenchb -f workloads/B/automatic.f;}
;;
manual)
RUNA() { ${RUN} exec -T filebencha job python benchmark.py -- filebencha -f workloads/A/manual.f;}
RUNB() { ${RUN} exec -T filebenchb job python benchmark.py -- filebenchb -f workloads/B/manual.f;}
;;
*)
